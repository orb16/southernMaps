...
```{r}
isle <- detailed_nz_islands %>% filter(grepl("codfish", tolower(name)))#find_my_island("codfish", proj = "nztm") #detailed_nz_islands %>% filter(grepl("codfish", tolower(name)))#
plot(isle)
```
...
```{r}
# multiple islands
many <- detailed_nz_islands %>% filter(grepl("auckland|campbell", tolower(grp_name)) |
