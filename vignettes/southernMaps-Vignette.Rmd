...
This is useful when you want to do spatial functions on your data, but also can be useful for plotting. Here we create a base R object, and also convert the sf map object to a SpatialPolygonsDataFrame object using `as_Spatial`. 
...
plot(nzSml84 %>% as_Spatial(), col  = "grey", border = NA)
...
```{r}
isle_codfish <- st_as_sf(isle) 

idf <- ggplot() +
  geom_sf(data = isle_codfish, colour = "grey",
          fill = "white") +
...
some_points <- st_sample(isle_codfish, size= 8) %>%
  st_as_sf(.) %>%
